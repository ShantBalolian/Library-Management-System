...
class Customer:
    def __init__(self, root):
        ...
        path_to_file = "C:/Users/reals/OneDrive/Desktop/CPSPROJECT/home-libraries-0516-AD-KOTU05-01.jpg"
        self.bg=ImageTk.PhotoImage(Image.open(path_to_file))
        self.bg_image = Label(self.root, image=self.bg).place(x=500, y=0, relwidth=1, relheight=1)
        ...
        self.book_title = Entry(frame1, font=("times new roman", 15), bg = "lightgray")
        self.book_title.place(x=160, y=100, width = 250, height=30)

        #=========== Author
        author_lbl = Label(frame1, text="Author or Director", font=("times new roman", 15, "bold"), bg="white", fg="gray").place(x=160, y=140)
        ...
        self.item_number = Entry(frame1, font=("times new roman", 15), bg = "lightgray")
        self.item_number.place(x=160, y=240, width = 250, height=30)
        ...
        logout= Button(self.root, text="Logout",fg="white", bg="#0B547C", font=("Arial", 15, "bold"), command = self.logout).place(x=580, y=400, width = 350, height=30)
        ...
        self.listBox.place(x=510, y=20, width=480, height =350)
    ...
    def search(self):
        ...
        if self.genre and self.type_select:
            ...
            if self.genre.get()!="All" and self.type_select.get()=="BOTH":
                ...
                cursor.execute(query.genre_search.format(genre, genre))
            ...
            if self.item_number.get():
                # print(self.item_number.get())
                #for i in self.listBox.get
                self.remove_all()
                item = self.item_number.get()
                cursor.execute(query.item_number.format(item, item))
                tables = cursor.fetchall()
                for table in tables:
                    self.listBox.insert("", "end", values=(table[0],table[1], table[2], table[3], table[4]))
            elif self.txt_author.get():
                self.remove_all()
                author = self.txt_author.get()
                cursor.execute(query.author.format(author, author))
                tables = cursor.fetchall()
                for table in tables:
                    self.listBox.insert("", "end", values=(table[0],table[1], table[2], table[3], table[4]))
            elif self.book_title.get():
                self.remove_all()
                title = self.book_title.get()
                cursor.execute(query.author.format(title, title))
                tables = cursor.fetchall()
                for table in tables:
                    self.listBox.insert("", "end", values=(table[0],table[1], table[2], table[3], table[4]))
